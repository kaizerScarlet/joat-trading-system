...
def test_default_initialization():
    fee : FeeScheduleProtocol = FeeSchedule()
...
def test_custom_initialization():
    fee : FeeScheduleProtocol = FeeSchedule(maker_bps=5.5, taker_bps=12.3)
...
def test_zero_fees():
    fee : FeeScheduleProtocol = FeeSchedule(maker_bps=0.0, taker_bps=0.0)
    assert fee.maker_rate() == 0.0
    assert fee.taker_rate() == 0.0

def test_negative_fees():
    fee : FeeScheduleProtocol = FeeSchedule(maker_bps=-5.0, taker_bps=-10.0)
    assert fee.maker_rate() == -0.0005
    assert fee.taker_rate() == -0.0010

def test_high_precision_fees():
    fee : FeeScheduleProtocol = FeeSchedule(maker_bps=7.777, taker_bps=9.999)
