...
def test_default_initialization():
    model : LatencyModelProtocol = LatencyModel()
...
def test_custom_initialization():
    model : LatencyModelProtocol = LatencyModel(base_ms=10.0, jitter_ms=5.0, p_tail=0.1, tail_multiplier=2.0)
...
def test_sample_ms_within_expected_range():
    model : LatencyModelProtocol = LatencyModel(base_ms=20.0, jitter_ms=10.0, p_tail=0.0)
    for _ in range(1000):
        latency = model.sample_ms()
        assert 10 <= latency <= 30  # base ± jitter

def test_tail_latency_triggered():
    model : LatencyModelProtocol = LatencyModel(base_ms=20.0, jitter_ms=0.0, p_tail=1.0, tail_multiplier=3.0)
...
def test_tail_latency_never_triggered():
    model : LatencyModelProtocol = LatencyModel(base_ms=20.0, jitter_ms=0.0, p_tail=0.0, tail_multiplier=3.0)
    for _ in range(100):
        latency = model.sample_ms()
        assert latency == 20  # No tail

def test_latency_never_negative():
    model : LatencyModelProtocol = LatencyModel(base_ms=5.0, jitter_ms=10.0)
    for _ in range(1000):
        latency = model.sample_ms()
        assert latency >= 0
...
def test_tail_probability_effect():
    model_low_tail : LatencyModelProtocol = LatencyModel(base_ms=20.0, jitter_ms=0.0, p_tail=0.01, tail_multiplier=3.0)
    model_high_tail : LatencyModelProtocol = LatencyModel(base_ms=20.0, jitter_ms=0.0, p_tail=0.5, tail_multiplier=3.0)
